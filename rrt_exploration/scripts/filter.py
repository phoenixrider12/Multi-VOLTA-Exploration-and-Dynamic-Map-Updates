...
# ---------------------------------------------------------------------------------------------------------------
    ...
    if len(namespace) > 0:
        for i in range(0, n_robots):
            rospy.loginfo(namespace+str(i+namespace_init_count) + global_costmap_topic)
            rospy.Subscriber(namespace+str(i+namespace_init_count) + global_costmap_topic, OccupancyGrid, globalMap)
...
# wait if any of robots' global costmap map is not received yet
    ...
    if len(namespace) > 0:
        for i in range(0, n_robots):
            rospy.loginfo(global_frame[1:])
            rospy.loginfo(namespace[1:]+str(i+namespace_init_count)+'/'+robot_frame)
            tfLisn.waitForTransform(global_frame[1:], namespace[1:]+str(i+namespace_init_count)+'/'+robot_frame, rospy.Time(0), rospy.Duration(10.0))
